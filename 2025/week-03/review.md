...
**Терминально-ориентированная аннотация**

- **Применение**: Используется в первых двух раундах обучения, когда модель PPM еще недостаточно надежна.
- **Обновление Q-значений**: Q-значение шага $s_i$ в $k$-м развертывании $q(s_i)_k$ обновляется на основе Q-значения терминального узла $s_d$ $q(s_d)$.
...
- **Формула UCT**: Для выбора узла $s$ используется формула UCT (Upper Confidence bounds for Trees):
  ...
  - $Q(s)$ – среднее Q-значение узла $s$. $Q(s) = \frac{q(s)}{N(s)}$, где $q(s)$ - это текущее значение Q-значения, а $N(s)$ - количество посещений узла $s$.
  - $N(s)$ — количество посещений узла $s$.
  - $N_{\text{parent}}(s)$ – количество посещений родительского узла $s$.
  - $c$ – константа, балансирующая между исследованием и использованием (exploration and exploitation).

- **Баланс исследования и использования**: Выбор узла с наивысшим значением UCT обеспечивает баланс между исследованием новых, возможно, более перспективных путей (exploration) и использованием уже известных, хорошо зарекомендовавших себя путей решения (exploitation).

Таким образом, Q-значения играют ключевую роль в MCTS, направляя поиск решения в сторону наиболее перспективных траекторий.
