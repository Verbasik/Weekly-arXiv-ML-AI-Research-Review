...
class OptimizedMoELayer(nn.Module):
    ...
    def forward(
        ...
        #       1.1. Извлеките размерности: B, S, H = hidden_states.shape
        #       1.2. Сохраните K = self.top_k
        #       1.3. Expand токены для K выборов:
        #            tokens = hidden_states.unsqueeze(2).expand(B, S, K, H)
        #            Размерность: (B, S, H) → (B, S, 1, H) → (B, S, K, H)
        #       1.4. Flatten всё в 1D:
        #            tokens_flat = tokens.reshape(-1, H)         # (B*S*K, H)
