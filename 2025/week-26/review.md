...
<details> 
    <summary><em><strong>Новая парадигма в NLP</strong></em></summary>

### 1. Введение: Смена Парадигмы в Генерации Текста
...
В данном обзоре рассматриваются четыре ключевые архитектуры DLM: Gemini Diffusion (Google DeepMind), Mercury Coder (Inception Labs), LLaDA (китайские исследователи) и Eso-LM (NVIDIA и Корнеллский университет), их архитектурные особенности, механизмы диффузии, алгоритмы генерации, а также экспериментальные результаты по качеству и производительности.

### 2. Архитектурный Анализ: Ключевые Модели Diffusion LM

#### 2.1. Gemini Diffusion (Google DeepMind)

**Gemini Diffusion** — экспериментальная текстовая модель от Google DeepMind, использующая диффузионный подход для генерации целых фрагментов текста и их итеративного улучшения.

**Механизм диффузии:** Модель обучается преобразовывать случайный шум в осмысленный текст. В процессе forward случайный шум (маскирование или замена токенов) постепенно повреждает исходный текст. В процессе reverse модель шаг за шагом удаляет шум, восстанавливая исходное содержимое. Каждый шаг денойзинга — это проход трансформера.

**Генерация и архитектура:** Начинает с полностью зашумленной последовательности фиксированной длины и преобразует ее в осмысленный ответ за несколько итераций. Все токены обновляются параллельно, генерируя целый блок текста одновременно.

**Скорость:** Внутренние оценки Google показывают скорость 1000–2000 токенов/с, что значительно превосходит AR-модели Gemini Flash (~272 токена/с).

**Бидирекциональное внимание:** При денойзинге токены видят контекст с обеих сторон, что улучшает глобальную связность фразы и позволяет учитывать будущие слова при выборе ранних.

**Инженерные новшества:** Использует специальное расписание восстановления токенов и модифицированный трансформер с полным вниманием по всему блоку для повышения когерентности. Режим Instant Edit позволяет редактировать существующий текст "на лету", исправляя грамматику или меняя стиль.

#### 2.2. Mercury Coder (Inception Labs)

**Mercury** — первая коммерческая масштабная диффузионная LLM, разработанная Inception Labs и предназначенная для генерации кода.

**Механизм диффузии:** Реализует coarse-to-fine генерацию. На первом этапе создается грубый набросок, который затем параллельно уточняется за несколько итераций, постепенно удаляя шум и заполняя пропуски.

**Алгоритм генерации:** Генерирует полный ответ за фиксированное число шагов денойзинга. Ключевая особенность — параллельность, позволяющая обновлять множество токенов за один шаг.

**Скорость:** Демонстрирует свыше 1000 токенов/с на GPU NVIDIA H100, что в 5–10 раз быстрее оптимизированных AR-аналогов (например, GPT-4o Mini ~59 токенов/с).

**Инновации:** Базируется на методе score-based diffusion для дискретных данных, используя score entropy для стабильного обучения. Модель учится оценивать отношение правдоподобий между правильным и текущим (зашумленным) токеном.

**Прогрессивное случайное маскирование:** На этапе forward все больший процент токенов маскируется. Reverse процесс начинается с полностью замаскированной заготовки и поэтапно открывает маски, используя адаптивное расписание восстановления: более легкие для предсказания элементы раскрываются раньше, сложные — позже. Это делает генерацию устойчивой и позволяет исправлять ошибки.

**Оптимизации:** Возможно, использует кеширование key-value активаций трансформера между шагами для повышения эффективности.

#### 2.3. LLaDA (Large Language Diffusion Models)

**LLaDA** — открытая исследовательская инициатива, доказавшая, что диффузионные модели могут конкурировать с авторегрессией на уровне больших LLM.

**Архитектура и механизм:** Это маскированная диффузионная модель (Masked Diffusion Model, MDM). Forward-процесс реализован через дискретное случайное маскирование токенов. Reverse-процесс — итеративное восстановление масок, где трансформер предсказывает токены на месте масок (выполняя Masked Language Modeling) с двунаправленным вниманием по всей последовательности. Модель оптимизирует нижнюю вариационную оценку правдоподобия (ELBO).

**Алгоритм генерации:** Генерирует последовательность за фиксированное число шагов $T$. На каждом обратном шаге $t$ модель параллельно заполняет поднабор масок. Порядок раскрытия масок задается случайно. Как только позиция открылась, она остается зафиксированной. Это обеспечивает параллельность и стабильность, позволяя общему числу сетевых обращений быть значительно меньше длины последовательности ($T \ll L$).

**Инженерные достижения:** Первая диффузионная модель, успешно масштабированная до 8 миллиардов параметров с обучением от нуля на 2.3 трлн токенов. LLaDA решает проблему "curse of diffusion in discrete space", демонстрируя масштабируемость и ключевые способности LLM (обобщение знаний, понимание инструкций) на уровне AR-моделей. LLaDA-8B успешно выполняет сложные интерактивные задачи после SFT и показала превосходство над GPT-4o в задачах reverse reasoning, требующих двунаправленной обработки.

#### 2.4. Eso-LM (NVIDIA & Cornell)

**Eso-LM (Esoteric Language Models)** — экспериментальная гибридная архитектура, объединяющая авторегрессию и диффузию для достижения лучшего качества и скорости.

**Двухфазная генерация:** Процесс разделен на диффузионную и последовательную фазы. Сначала MDM-модель параллельно генерирует черновик фразы, оставляя часть позиций замаскированными. Затем AR-модель последовательно заполняет оставшиеся маски слева направо. Параметр $\alpha_0$ контролирует пропорцию: $\alpha_0=1$ для чистой диффузии, $\alpha_0=0$ для чистой AR.

> "Eso-LMs ... успешно объединив парадигмы авторегрессионных (AR) и моделей маскированной диффузии (MDM)."
>
> "Фаза диффузии (оранжевый) постепенно удаляет шум из токенов параллельно, в то время как последовательная фаза (зеленый) заполняет оставшиеся маскированные токены авторегрессивно".

**Единая архитектура и внимание:** Обучается один трансформер, способный работать в обоих режимах благодаря кастомному механизму внимания с матрицей смещений $A$.

**Eso-LM (A):** Использует двунаправленное внимание между чистыми токенами и причинное внимание к маскированным в фазе диффузии.

**Eso-LM (B):** Вводит каузальное ограничение даже для «чистых» токенов во время диффузии, что позволяет кешировать KV-активации для них, обеспечивая ускорение до 65% по сравнению с базовым MDM без кеширования.

**Итоги архитектуры:** Eso-LM продемонстрировал, что симбиотическое объединение AR и диффузии возможно. Модель достигла лучшей на сегодня перплексии среди диффузионных моделей и предоставляет непрерывный спектр качество/скорость между AR и MDM, попадая на новую Pareto-границу качества и быстродействия.

> "Это лучший результат среди нелинейных (не-AR) методов на данных корпусах."

### 3. Математическая и Вероятностная Формализация

Все рассмотренные DLM стремятся моделировать распределение вероятностей над текстом через марковский диффузионный процесс, а не авторегрессионное разложение $P(X) = \prod_{i=1}^L P(x_i \mid x_{<i})$.

#### 3.1. Кросс-энтропия и ее роль

Кросс-энтропия ($H(P, Q) = -\sum_{k=1}^K P(k) \log Q(k)$) является центральной функцией потерь в задачах классификации. Она измеряет "удивление" модели относительно истинного распределения. В контексте машинного обучения, минимизация кросс-энтропии эквивалентна максимизации логарифма правдоподобия. Для one-hot разметки кросс-энтропия равна KL-дивергенции ($D_{KL}(P \mid Q) = H(P, Q) - H(P)$), так как энтропия истинного one-hot распределения $H(P)$ равна нулю.

#### 3.2. Суть Диффузионной Модели для Текста

В DLM вводится скрытая переменная — индекс шага диффузии $t$.

**Прямой (forward) процесс $q$:** Постепенное добавление шума к данным, например, через маскирование токенов с возрастающей вероятностью $\beta_t$. При $t=T$ данные полностью разрушены. Этот процесс известен точно.

**Обратный (reverse) процесс $p_\theta$:** Обучаемый процесс удаления шума. Нейронная сеть (параметризованная весами $\theta$) аппроксимирует распределение восстановления $p_\theta(x_{t-1} \mid x_t)$ — вероятность получить менее зашумленное состояние $x_{t-1}$ из текущего $x_t$.

Для вычислительной разрешимости предполагается полная факторизация этого распределения по позициям, содержащим шум на шаге $t$: 

$$ p_\theta(x_{t-1} \mid x_t) = \prod_{i\in \mathcal{M}_t} p_\theta(x^{(i)}_{t-1} \mid x_t) $$

Это означает, что модель предсказывает каждый замаскированный токен независимо (условно на текущее состояние всей последовательности $x_t$). Это эквивалентно задаче маскированного языкового моделирования (MLM), подобной BERT.

**Обучение:** Каждый шаг диффузии оптимизируется с помощью кросс-энтропии на правильный токен вместо маски. Функция потерь принимает вид Negative ELBO (NELBO) — отрицательной вариационной нижней оценки лог-правдоподобия данных. 
...
Где $X^{\mathcal{M}_t}$ — истинные токены на замаскированных позициях, а $w_t$ — весовые коэффициенты.

Mercury оптимизирует аналог ELBO через score entropy, обучая восстановление соотношений вероятностей. Eso-LM ввел гибридную модель правдоподобия, где общая функция потерь распадается на NELBO диффузионной части и обычную кросс-энтропию AR-части.

**Ключевой вывод:** DLM расширяют пространство решений для моделирования $P(X)$, подтверждая, что ключевые свойства LLM обеспечиваются мощью генеративного моделирования с максимизацией правдоподобия через иную факторизацию.

### 4. Экспериментальные Результаты

#### 4.1. Качество Генерации и Метрики

Современные DLM уже достигают сопоставимого с AR-трансформерами качества текста.

**LLaDA-8B:** По перплексии и zero-shot задачам не уступает AR-LLM аналогичного размера (LLaMA2-7B), приближаясь к LLaMA3-8B. Решила проблему “reversal curse” лучше, чем GPT-4(open).

**Gemini Diffusion:** По качеству сопоставима с гораздо более крупными моделями Gemini 2.x, почти не уступая Gemini Flash-Lite на кодовых бенчмарках (HumanEval ~89.6% vs 90.2%). Зато опередила в математическом тесте AIME 2025 (23.3% vs 20.0%). Отстает в MMLU и Big-Bench Hard.

**Mercury Coder:** Уверенно обошел Gemini 2.0 Flash-Lite, Claude 3.5 Haiku, GPT-4o Mini и Qwen 2.5 Coder на 4 из 6 стандартных бенчмарков по программированию. Находится в преференциально выгодной зоне (Pareto-optimal) по соотношению "качество решения vs токенов/с".

**Eso-LM:** Достиг перплексии 26.2 на LM1B и ~30.5 на OpenWebText, что лучше, чем у прежних диффузионных LM, и приближает его к AR-моделям. Генерирует более осмысленные и разнообразные тексты, не проявляя модового коллапса.

#### 4.2. Скорость, Латентность и Эффективность

Главное преимущество DLM — скорость генерации, особенно на длинных ответах.

**Gemini Diffusion:** Отвечает на запросы за 1–3 секунды против ~7 секунд у Gemini 2.5 Flash. Производительность 600–1300 токенов/с, максимальные заявленные значения 1000–2000 токенов/с, в разы превосходящие GPT-3/4 (100–200 токенов/с).

**Mercury:** Показал ~1100 токенов/с на H100, что сопоставимо с лучшими специализированными аппаратными ускорителями, но на стандартных GPU, подчеркивая эффективность алгоритма.

**Латентность:** DLM выгодны при генерации больших фрагментов текста. Время растет ближе к $O(\text{const})$ за счет фиксированного числа итераций, тогда как у AR-моделей линейно $O(N)$.

**Время до первого токена (TTFT):** Обычно выше у DLM, так как требуется завершить весь цикл итераций для получения полного ответа, в отличие от AR, выдающей первый символ сразу.

**Энергоэффективность:** Диффузионная генерация лучше параллелится и полнее задействует GPU. "Энергия на один токен" у DLM потенциально меньше из-за более высокой производительности. Адаптивные механизмы позволяют DLM экономить ресурсы на простых запросах.
...
| LLaDA 8B                      | Нет точных данных (потенциал $T \ll L$)| —                                      | Силён в zero-shot и инструкциях, превосходит GPT-4o в "reverse" задачах   |
| Eso-LM (A/B)                  | До 65% ускорения (B)  | Близко к AR                            | Лучший компромисс скорость/качество на длинах ~1K токенов                 |

### 5. Сравнение с Классическими Трансформерами

#### 5.1. Преимущества Парадигмы Diffusion LM

- **Высокая скорость и низкая задержка на длинных выводах:** Параллельная генерация позволяет DLM быть на порядок быстрее, делая LLM пригодными для реально-временных приложений (чат-боты, голосовые ассистенты, IDE-помощники).
- **Итеративное улучшение и самокоррекция:** DLM могут исправлять ошибки и галлюцинации на последующих шагах денойзинга, в отличие от необратимого выбора токена в AR-моделях.
- **Более глобальная когерентность текста:** Бидирекциональное внимание позволяет учитывать контекст будущих слов при генерации предыдущих, улучшая согласованность текста в целом.
- **Гибкость и редактируемость:** DLM естественно подходят для интерактивного редактирования, дополнения в середине текста, стилистической трансформации, поскольку их обучение основано на MLM.
- **Адаптивные вычисления:** DLM способны динамически менять число итераций генерации в зависимости от сложности задачи, потенциально обеспечивая более эффективное использование вычислений.

#### 5.2. Ограничения и Вызовы

- **Сложность и стоимость обучения:** Требуются большие наборы данных и вычислительные ресурсы (LLaDA 8B обучалась на 2.3 трлн токенов, 130 тыс. GPU-часов). Оптимизация ELBO с многократным маскированием сложнее, чем простая кросс-энтропия AR.
- **Время до первого символа:** DLM выдают весь блок текста сразу, что может казаться медленным для сценариев, где пользователь ожидает увидеть поток текста (например, имитация набора).
- **Выделение памяти и вычислений:** На каждом шаге DLM обрабатывают весь контекст, что может потребовать больше памяти. Однако разработки вроде Eso-LM (B) показывают возможность кеширования.
- **Точность на детальных задачах:** В некоторых случаях AR-модели показывают лучшее качество на задачах, требующих точных знаний или многошагового логического вывода (например, MMLU у Gemini Diffusion).

#### 5.3. Потенциал Замещения AR-моделей

Многие эксперты считают, что диффузионный подход может стать новым стандартом для LLM. Google открыто заявляет о намерении исследовать этот путь. Появление Mercury в коммерческом поле подтверждает интерес индустрии к быстрым решениям. Преимущества по скорости и масштабируемости слишком значимы. Гибридные варианты, такие как Eso-LM, могут послужить плавным мостом, используя лучшее от обоих подходов и предлагая спектр моделей с разным соотношением скорость/точность.

### 6. Заключение и Перспективы

Диффузионные языковые модели — многообещающий шаг вперед в NLP, доказывающий, что итеративная денойзинг-парадигма способна достичь сопоставимых результатов с AR-LLM при значительно лучшей производительности.

Парадигмальный сдвиг возможен, но потребуется время. Вероятно сосуществование AR (для пошагового предсказания, максимальной точности на малых моделях) и Diffusion (для длинных текстов, интерактивных сценариев).

Масштабируемость обнадеживает. Нет теоретических барьеров для обучения DLM с 70B+ параметров. Если Gemini Diffusion будет масштабирован до уровня GPT-4 и сохранит преимущество в скорости, это закрепит парадигму.

Практическое применение начнется с узких областей. Mercury выбрал кодогенерацию. Ожидается применение в системах реального времени (автоперевод речи, AR-подсказки, чат-боты) и корпоративных решениях благодаря способности к точечному редактированию.

Перспективные архитектуры: Gemini Diffusion может стать локомотивом. Mercury показал ценность новых оптимизационных целей (score entropy). LLaDA важна как доказательство внедрения DLM open-source сообществом. Eso-LM указывает направление гибридных решений и уже демонстрирует оптимальный компромисс скорости и качества с KV-кешем.

Диффузионные языковые модели сегодня находятся на грани превращения из научного эксперимента в новый стандарт индустрии. Соревнование двух подходов ведет к лучшим решениям, и в ближайшие годы ожидается появление новых гибридных парадигм, объединяющих сильные стороны всех подходов.
