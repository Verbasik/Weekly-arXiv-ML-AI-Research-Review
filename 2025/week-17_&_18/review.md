...
### 2.4 Пошаговая визуализация: Схема работы с пояснениями

Давайте визуализируем работу LSTM ячейки, шаг за шагом, чтобы лучше понять, как взаимодействуют все компоненты.

**Общая схема LSTM ячейки:**
...
h_{t-1}──┤ Конкатенация
    ...
    └──────────┬──────┘
          ...
          └────┬────┘
               │
C_{t-1}────►(×)───► Часть информации 
                    из C_{t-1} сохраняется
```

- Входы: $x_t$ (текущий вход) и $h_{t-1}$ (предыдущее скрытое состояние)
- Операция: $f_t = \sigma(W_f[x_t, h_{t-1}] + b_f)$
- Результат: вектор $f_t$ со значениями между 0 и 1
- Интерпретация: $f_t$ поэлементно умножается на $C_{t-1}$, определяя, какая информация из предыдущего состояния ячейки сохраняется

**Шаг 2: Вентиль входа (Input Gate) и кандидат-вектор**

```
x_t ──┐                         x_t ──┐
      │                               │
h_{t-1}──┤ Конкатенация         h_{t-1}──┤ Конкатенация
      │                               │
      ▼                               ▼
    ┌─────────────────┐             ┌─────────────────┐
    │ W_i[x_t,h_{t-1}]+b_i │             │ W_C[x_t,h_{t-1}]+b_C │
    └──────────┬──────┘             └──────────┬──────┘
               │                               │
               ▼                               ▼
          ┌─────────┐                     ┌─────────┐
...
- Операции:
  - $i_t = \sigma(W_i[x_t, h_{t-1}] + b_i)$ — какую новую информацию добавить
  - $\tilde{C}_t = \tanh(W_C[x_t, h_{t-1}] + b_C)$ — кандидат новых значений
- Результат: $i_t \odot \tilde{C}_t$ — отфильтрованная новая информация
- Интерпретация: $i_t$ определяет, какие элементы кандидат-вектора $\tilde{C}_t$ будут добавлены в состояние ячейки
...
- Операция: $C_t = f_t \odot C_{t-1} + i_t \odot \tilde{C}_t$
- Результат: Обновленное состояние ячейки $C_t$
- Интерпретация: Комбинация сохраненной старой информации и добавленной новой

**Шаг 4: Вентиль выхода (Output Gate) и вычисление скрытого состояния**
...
- Операции:
  - $o_t = \sigma(W_o[x_t, h_{t-1}] + b_o)$ — определяет, какую часть состояния ячейки отправить на выход
  - $h_t = o_t \odot \tanh(C_t)$ — фильтрованное скрытое состояние
- Результат: Новое скрытое состояние $h_t$
- Интерпретация: $o_t$ контролирует, какие части состояния ячейки $C_t$ будут видны внешнему миру и переданы на следующий шаг

**Полная схема LSTM ячейки:**

Теперь соединим все четыре шага в одну полную схему LSTM ячейки:
...
**Пример пошаговой работы LSTM на конкретном случае**

Рассмотрим упрощенный пример работы LSTM при обработке фразы: "Я вырос во Франции... Я бегло говорю по-"

Предположим, что на последнем слове "по-" модель должна предсказать "французски", используя информацию о Франции, упомянутой намного раньше.

1. **При обработке "Я вырос во Франции"**:
   - Вентиль входа ($i_t$) активируется сильно для информации о "Франции"
   - Состояние ячейки $C_t$ сохраняет информацию о местоположении "Франция"
   - Эта информация продолжает храниться в $C_t$ на следующих шагах

2. **Промежуточные токены** (представленные многоточием):
   - Вентиль забывания ($f_t$) сохраняет информацию о "Франции" в $C_t$
   - Другая менее важная информация может быть "забыта"

3. **При обработке "Я бегло говорю по-"**:
   - Контекст подсказывает, что далее следует название языка
   - Вентиль выхода ($o_t$) активируется для компонента $C_t$, содержащего информацию о "Франции"
   - Эта информация включается в $h_t$ и используется для правильного предсказания слова "французски"

Благодаря механизму ячейки памяти, LSTM способен "запомнить" Францию на длительный период и использовать эту информацию, когда она становится актуальной, что было бы невозможно для простой RNN из-за проблемы исчезающего градиента.
...
Вентили LSTM можно сравнить с регуляторами тока в электрической цепи:
- Сигмоидальная функция аналогична потенциометру, который плавно регулирует "силу тока" (поток информации)
- Значение 0 эквивалентно разомкнутой цепи (ток не идет)
- Значение 1 эквивалентно замкнутой цепи (ток идет без препятствий)
- Промежуточные значения создают различное "сопротивление" в информационном потоке
...
```
Ванильная RNN:
h_{t-k} → h_{t-k+1} → ... → h_t → L
      ↘↗ tanh'·W  ↘↗ tanh'·W  ↘↗ 
      Градиент затухает экспоненциально

LSTM:
C_{t-k} → C_{t-k+1} → ... → C_t → L
      ↘↗ f_{t-k+1}  ↘↗ f_t  ↘↗
      Градиент контролируется вентилями
```
...
### 4.1 Peephole Connections: Идея и реализация

**Peephole Connections** (сквозные соединения) — это важное расширение стандартной архитектуры LSTM, предложенное Феликсом Герсом и Юргеном Шмидхубером в 2000 году. Основная идея состоит в том, чтобы дать вентилям "заглянуть" непосредственно в состояние ячейки, что повышает их способность принимать более информированные решения.
