...
Этот документ содержит обзор архитектуры **Mamba**, новой модели для моделирования последовательностей, представленной Альбертом Гу (Университет Карнеги-Меллона) и Три Дао (Принстонский университет). Mamba представляет собой значительный шаг вперед, преодолевая основные ограничения существующих подходов, в частности, квадратичную вычислительную сложность Transformer-моделей при обработке длинных последовательностей.

### **1. Проблема Transformer-моделей и их альтернативы**

**Проблема:** Доминирующие в области моделирования последовательностей Transformers страдают от "узкого места внимания", при котором вычислительная сложность масштабируется квадратично с длиной последовательности. Это делает их неэффективными для обработки очень длинных последовательностей.

**Альтернативы и их ограничения:** Были предложены различные подходы для достижения линейного масштабирования (линейное внимание, сверточные алгоритмы, RNN, SSM), но им часто не хватает способности к содержательному рассуждению, свойственной механизмам внимания. State Space Models (SSM) показали потенциал для моделирования долгосрочных зависимостей, но традиционные SSM используют фиксированные параметры, не зависящие от входного содержимого, что ограничивает их способность выполнять задачи, требующие избирательного копирования и индукционных головок.

### **2. Архитектура Mamba: Селективные State Space Models (SSM)**

**Ключевая инновация:** Mamba вводит концепцию **селективных SSM**, где параметры модели (A, B, C, D) становятся функциями входных данных, а не фиксированными значениями. Это позволяет модели динамически решать, какую информацию хранить и распространять на основе содержимого текущих входных данных.

**Реализация селективности:** Параметры SSM генерируются динамически на каждом шаге с помощью проекционных слоев (гиперсетей), которые "смотрят" на текущий входной сигнал $x(t)$ и выдают соответствующие значения параметров. Этот механизм позволяет объединить линейное масштабирование SSM с содержательным рассуждением.

**Непрерывная во времени SSM:** Описывается дифференциальными уравнениями:
$$
\begin{align}
h'(t) &= Ah(t) + Bx(t) \\
y(t) &= Ch(t) + Dx(t)
...
В селективной формулировке параметры становятся зависимыми от входа: $A(x), B(x), C(x), D(x)$.

**Дискретизированная версия (используемая в Mamba):**
$$
...
y_t = C_t h_t + D_t x_t
$$
где параметры с чертой ($\bar A_t, \bar B_t$) и $C_t, D_t$ вычисляются на основе $x_t$.

**Архитектура модели:** Mamba использует чередование слоев селективных SSM и простых проекций, что делает ее структуру более простой по сравнению с Transformers, не требуя отдельных блоков внимания или MLP.

### **3. Механизм отбора и его возможности**

Механизм отбора является центральным элементом Mamba, обеспечивающим содержательное рассуждение. Он обрабатывает текущий входной токен $x_t$ и вычисляет параметры SSM, определяющие поток информации.

**Ключевые возможности, реализуемые механизмом отбора:**
- **Выборочное копирование:** Способность избирательно извлекать и копировать конкретную информацию из последовательности.
- **Головы индукции:** Способность распознавать паттерны и экстраполировать их на новые контексты.
- **Превосходная экстраполяция:** Mamba демонстрирует выдающуюся способность экстраполировать на значительно более длинные последовательности, чем те, на которых она обучалась, сохраняя высокую точность даже при масштабировании до миллиона токенов, в отличие от других моделей линейной сложности.

### **4. Реализация с учетом аппаратного обеспечения**

**Вычислительная проблема:** Динамическое изменение параметров SSM не позволяет использовать стандартные быстрые сверточные алгоритмы.

**Решение:** Авторы разработали аппаратно-ориентированный алгоритм параллельного сканирования. Этот алгоритм эффективно выполняет рекуррентные вычисления, тщательно управляя памятью (используя быструю память SRAM графического процессора для расширенного состояния), что значительно снижает затраты на доступ к памяти.

**Результат:** Реализация Mamba значительно превосходит существующие подходы по скорости вычислений для операций с последовательностями и масштабируется линейно с длиной последовательности.

### **5. Анализ производительности и универсальность**

**Моделирование языка:** Mamba демонстрирует сопоставимую или превосходящую производительность по сравнению с Transformer-моделями аналогичного размера, следуя тем же законам масштабирования, но с большей эффективностью параметров. Перплексия Mamba-3B превосходит Transformer того же размера и достигает показателей Transformer, вдвое большего по размеру.

**Перплексия (PPL):** Метрика оценки языковых моделей, измеряющая, насколько хорошо модель предсказывает следующее слово. Чем ниже значение, тем лучше модель.
$$
PPL = \exp\left(-\frac{1}{N} \sum_{i=1}^{N} \log p(w_i | w_1, \dots, w_{i-1})\right)
$$

**Обработка аудио:** Достигает передовых результатов в задачах моделирования аудиосигналов, превосходя специализированные модели.

**Геномика:** Высокая производительность как в задачах моделирования языка ДНК, так и в задачах классификации видов, особенно благодаря способности использовать долгосрочные зависимости.

**Универсальность:** Одна из наиболее убедительных особенностей Mamba — ее способность достигать высокой производительности в различных модальностях (язык, аудио, геномика) без специфических для домена модификаций. Это свидетельствует о том, что Mamba охватывает фундаментальные возможности моделирования последовательностей.

### **6. Свойства масштабирования и эффективность вывода**

**Масштабирование длины последовательности:** Эффективность Mamba улучшается с увеличением длины контекста, в отличие от Transformer-моделей. Это критически важно для задач с очень длинными последовательностями.

**Масштабирование размера модели:** Производительность Mamba плавно масштабируется с размером модели, следуя предсказуемым законам масштабирования.

**Эффективность вывода:** Mamba демонстрирует значительные преимущества в скорости вывода, достигая примерно в 5 раз более высокой пропускной способности по сравнению с Transformer-моделями сопоставимого размера на стандартном оборудовании. Линейное масштабирование с длиной последовательности усиливает это преимущество при увеличении контекстных окон.

### **7. Ограничения и будущая работа**

- **Двунаправленность:** Текущая архитектура в основном каузальна. Требуются исследования для расширения на двунаправленные конфигурации.
- **Интерпретируемость:** Селективный механизм усложняет внутреннюю динамику.
- **Мультимодальная интеграция:** Применение Mamba для мультимодальных задач требует изучения.
- **Эффективность параметров:** Возможно дальнейшее повышение эффективности параметров.

Будущая работа может включать изучение более сложных механизмов отбора, альтернативных параметризаций SSM и применение в новых областях, таких как обучение с подкреплением.

### **Заключение**

Mamba представляет собой прорыв, сочетая линейное масштабирование SSM с возможностями содержательного рассуждения, ранее присущими только механизмам внимания. Это устраняет фундаментальное ограничение существующих моделей и открывает новые возможности для эффективных моделей, особенно при работе с длинными последовательностями. Универсальность и вычислительная эффективность Mamba делают ее многообещающей основой для следующего поколения систем ИИ, работающих с последовательностями.
...
## **2. Понимание традиционного моделирования последовательностей**
...
## **5. Реализация с учетом аппаратного обеспечения**
...
Перплексия (perplexity, PPL) — это метрика, используемая для оценки качества языковых моделей. Она измеряет, насколько хорошо модель предсказывает следующее слово в последовательности. Чем ниже значение перплексии, тем лучше модель предсказывает текст.  

Формально перплексия определяется как экспонента от средней отрицательной лог-вероятности последовательности слов:  
...
- $(p(w_i | w_1, \dots, w_{i-1}))$ — вероятность предсказания слова $(w_i)$ на основе предыдущих слов.  

#### **Как интерпретировать перплексию?**  
- **PPL = 1** — идеальная модель (всегда угадывает следующее слово с вероятностью 1).  
- **PPL = k** — модель в среднем колеблется между $(k)$ равновероятными вариантами.  
- Чем ниже PPL, тем увереннее модель в своих предсказаниях. 
...
*Рисунок 6: Законы масштабирования при моделировании языка, демонстрирующие высокую производительность Mamba по сравнению с различными архитектурами Transformer и альтернативными архитектурами.*
...
*Рисунок 7: Сравнение производительности моделирования аудиосигнала при различных параметризациях модели. Mamba демонстрирует последовательные улучшения по мере увеличения длины последовательности.*
...
*Рисунок 8: Законы масштабирования при моделировании последовательности генома человека. Mamba превосходит другие архитектуры по мере увеличения числа параметров.*
...
## **7. Универсальность в разных областях**
...
*Рисунок 9: Законы масштабирования относительно длины последовательности в данных YouTubeMix. Mamba последовательно превосходит S4+FFN по мере увеличения длины последовательности.*
...
*Рисунок 10: Масштабирование сложности с вычислительными показателями FLOP для различных вариантов Mamba, демонстрирующее последовательное улучшение по мере увеличения вычислительной мощности.*
...
*Рисунок 11: Сравнение пропускной способности вывода между моделями Mamba и Transformer. Mamba достигает значительно более высокой пропускной способности для всех размеров партий.*
