...
Чтобы избежать коллапса маршрутизации (например, когда все отправляется одним и тем же экспертам), DeepSeek-V2 имел специальную балансировочную потерю, на самом деле две: одну на уровне экспертов, другую на уровне вычислительного устройства, что имеет смысл, поскольку баланс желателен в обоих местах.
...
Слишком большая потеря может повредить производительности модели, и в DeepSeek-V3 они отказались от дополнительной потери, используя специальную стратегию балансировки нагрузки без дополнительных потерь , опубликованную командой немного ранее ( https://arxiv.org/abs/2408.15664 ). В ней смещение добавляется к оценке сродства во время маршрутизации, а top-k берется из результата. Это смещение не используется для вычисления экспертного коэффициента смешивания (значения стробирования).

![Figure_8](https://raw.githubusercontent.com/Verbasik/Weekly-arXiv-ML-AI-Research-Review/refs/heads/develop/2025/week-07-08/assets/Figure_8.jpg)

Специальная процедура контролирует изменение смещения, отслеживая, какие эксперты были вызваны в пакете, и если кто-то перегружен, его смещение понижается (и повышается, если эксперт бездействует). Работает лучше, чем с потерями.

![Figure_9](https://raw.githubusercontent.com/Verbasik/Weekly-arXiv-ML-AI-Research-Review/refs/heads/develop/2025/week-07-08/assets/Figure_9.jpg)

Интересный отход от обратного распространения ошибки. Хотя, возможно, они просто не нашли правильный подход к обучению с обратным распространением ошибки...

Чтобы избежать дисбаланса в обработанной последовательности, они также добавили Complementary Sequence-Wise Auxiliary Loss с очень малым весом. Существует алгоритмическая Node-Limited Routing , которая ограничивает устройства, концептуально похожая на балансировочную потерю в DeepSeek-V2. Каждый токен отправляется максимум на 4 узла.

### Прогнозирование нескольких токенов (MTP)

Далее новые функции. Используется Multi-Token Prediction (MTP) . Идея MTP заключается в прогнозировании более одного токена в каждой позиции. В текущей модели это два токена — текущий и следующий. Теоретически это усиливает обучающий сигнал и может повысить эффективность данных. Это также может помочь модели лучше подготовиться к прогнозированию будущих токенов.

![Figure_10](https://raw.githubusercontent.com/Verbasik/Weekly-arXiv-ML-AI-Research-Review/refs/heads/develop/2025/week-07-08/assets/Figure_10.jpg)

Прогнозирование токенов выполняется последовательно. Для прогнозирования D дополнительных токенов используются D модулей MTP, которые имеют общие вложения и выходную головку. Они получают выходные данные из основного слоя модели или предыдущего модуля MTP, а также вложения следующего токена, все нормализуется с помощью RMSNorm и объединяется. Каждый модуль вычисляет потерю кросс-энтропии, средняя потеря вычисляется по всем модулям, и она действует как дополнительная потеря модели с коэффициентом λ (0,3 для первых 10T токенов, 0,1 для последующих 4,8T). Во время вывода модули MTP отбрасываются, но их можно использовать для спекулятивного декодирования.

![Figure_11](https://raw.githubusercontent.com/Verbasik/Weekly-arXiv-ML-AI-Research-Review/refs/heads/develop/2025/week-07-08/assets/Figure_11.jpg)

MTP последовательно улучшает производительность на большинстве бенчмарков. В экспериментах скорость принятия следующего токена варьировалась от 85% до 90%. В сочетании со спекулятивным декодированием TPS увеличивается в 1,8 раза.

### Инфраструктура

Еще одна интересная часть — это инфраструктура.

DeepSeek-V3 обучался на кластере из 2048 графических процессоров NVIDIA H800. Напоминание: H800 — это ограниченный H100 для китайского рынка. У H800 сокращено межсоединение (пропускная способность уменьшена более чем вдвое, а количество связей NVLink также уменьшено), а FP64 FLOPS уменьшены в десятки раз — неважно для нейронных сетей, но хуже для расчета атомных бомб. Для «особо логической» нумерации H200 — это улучшенная версия H100 с большим объемом более быстрой памяти.

Для обучения компания написала собственную закрытую платформу HAI-LLM.

![Figure_12](https://raw.githubusercontent.com/Verbasik/Weekly-arXiv-ML-AI-Research-Review/refs/heads/develop/2025/week-07-08/assets/Figure_12.jpg)

DeepSeek-V3 использует 16-сторонний конвейерный параллелизм (PP), 64-сторонний экспертный параллелизм (EP) с 8 узлами и параллелизм данных ZeRO-1 (DP). Для эффективного PP они разработали алгоритм DualPipe, перекрывающий фазы коммуникации и вычисления в прямых и обратных проходах. Это приводит к уменьшению пузырей конвейера. Благодаря серьезной оптимизации памяти они обошлись без тензорного параллелизма (TP). Они также разработали эффективные межузловые ядра коммуникации all-to-all.
