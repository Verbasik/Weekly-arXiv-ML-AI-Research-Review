...
![Table_1](https://raw.githubusercontent.com/Verbasik/Weekly-arXiv-ML-AI-Research-Review/refs/heads/develop/2025/week-07/assets/Figure_1.jpg)
...
![Figure_2](https://raw.githubusercontent.com/Verbasik/Weekly-arXiv-ML-AI-Research-Review/refs/heads/develop/2025/week-07/assets/Figure_2.png)
...
#### Инновация MLA: Сжатие низкого ранга для ключей и значений
...
#### Оптимизация матриц проекций в MLA

Более того, в MLA матрицы проекции из скрытого вектора ($c_t$) в ключи и значения могут быть **устранены**.  Матрица для ключей ($W_{uk}$) может быть интегрирована в матрицу для получения запросов ($W_q$), а матрица для значений ($W_{uv}$) – в выходную матрицу ($W_o$).

#### Низкоранговое сжатие запросов

В DeepSeek-V3, векторы запросов ($q_t$) также подвергаются **низкоранговому сжатию** в собственный скрытый вектор ($c_t$).  Хотя это не влияет на размер KV-кэша, это способствует **снижению объема памяти для активаций** во время обучения, что также косвенно способствует повышению эффективности и масштабируемости модели.

#### Развязанная стратегия RoPE для позиционных вложений

В процессе разработки MLA возникла проблема несовместимости позиционных вложений RoPE (Rotary Positional Embeddings) с низкоранговым сжатием KV.  Для решения этой проблемы была предложена **развязанная стратегия RoPE**.  Эта стратегия вводит дополнительные многоголовочные запросы ($q_R$) и общие ключи ($k_R$) с собственной размерностью ($d^R_h$) на голову.  Окончательные векторы для запросов ($Q$) и ключей ($K$) формируются как **конкатенация** векторов, полученных из соответствующего низкорангового вектора ($c_t$) и векторов RoPE ($q_R, k_R$).

#### Детализация формул MLA

Для более детального понимания механизма MLA, рекомендуется обратиться к разделу 2.1.2 технического отчета DeepSeek-V3 [[6](https://arxiv.org/abs/2412.19437)], где представлены подробные формулы и математическое описание данного механизма внимания.  Визуальное представление и формулы могут быть более понятными, чем текстовое описание.
