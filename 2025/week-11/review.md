...
Основная идея дистилляции знаний заключается в переносе информации от большой модели-учителя к компактной модели-ученику. В этом процессе прогнозируемое распределение вероятностей модели-учителя используется в качестве целевого распределения для модели-ученика. Обучение происходит путем минимизации расхождения Кульбака-Лейблера (KL-дивергенции) между распределениями:
...
1. Закон масштабирования дистилляции, разработанный Apple и Оксфордским университетом, математически описывает, как качество модели-студента зависит от трёх факторов: её размера, количества обучающих данных и качества модели-учителя. Фундаментальный принцип гласит: студент никогда не превзойдет учителя ($L_S ≥ L_T$), где L — кросс-энтропия, меньшие значения означают лучшую модель. Закон позволяет точно предсказать, насколько близко к учителю приблизится студент при заданных параметрах, устраняя необходимость в дорогостоящих экспериментах.
...
2. Ключевое открытие исследования — "разрыв в способности к обучению", который выходит за рамки простого количества параметров и описывает фундаментальное несоответствие между моделями. Этот разрыв проявляется в двух аспектах: 

- **Гипотезном пространстве** — наборе концепций и абстракций, которые модель способна представлять; крупная модель оперирует сложными многоуровневыми абстракциями, недоступными для меньшей модели; 
- **Оптимизационной способности** — эффективности поиска оптимальных решений в пространстве параметров. Именно поэтому зависимость качества студента от размера учителя имеет U-образную форму: существует оптимальный размер учителя — не слишком маленький (недостаточно знаний) и не слишком большой (представления знаний слишком сложны для восприятия студентом, как профессор квантовой физики для первоклассника).

Исследователи статистически предлагают находить оптимальный разрыв в способности к обучению, формулируя **закон масштабирования дистилляции (Уравнение 8)**. Этот закон позволяет оценить, как размер учителя ($N_T$) и количество токенов его обучения ($D_T$), обобщенные через кросс-энтропию учителя ($L_T$), а также размер студента ($N_S$) и количество токенов его дистилляции ($D_S$), влияют на кросс-энтропию студента ($L_S$).

Вот как именно этот закон и сопутствующие эксперименты помогают выявить оптимальный разрыв:

*   **Закон масштабирования как модель зависимости:** Уравнение 8 представляет собой математическую модель, которая описывает сложную взаимосвязь между характеристиками учителя и студента и конечной производительностью студента. Включая $L_T$ в качестве ключевой переменной, закон отражает, что не только размер учителя, но и качество его знаний (определяемое $N_T$ и $D_T$) влияют на процесс дистилляции.
*   **U-образная зависимость и термин "capacity gap":** Закон масштабирования (Уравнение 8) эмпирически подтверждает U-образную зависимость качества студента от размера учителя. Это отражается в структуре формулы, где влияние $L_T$ на $L_S$ не является монотонным. Слишком слабый учитель не передаст достаточно знаний, а слишком сильный учитель может представлять знания в форме, слишком сложной для студента, что приводит к ухудшению производительности. Термин "capacity gap" (разрыв в возможностях) вводится для описания этого явления, которое закон масштабирования помогает количественно оценить.
*   **Экспериментальная верификация и подбор коэффициентов:** Исследователи провели обширные контролируемые эксперименты с учителями и студентами различных размеров и объемов данных для обучения и дистилляции. Данные этих экспериментов были использованы для статистической подгонки **положительных коэффициентов** $\{c_0, c_1, d_1, f_1, \alpha', \beta', \gamma', A, B\}$ в Уравнении 8. Точная подгонка этих коэффициентов позволяет закону масштабирования точно предсказывать производительность студента для невидимых комбинаций размеров учителя, студента и объемов данных.
*   **Анализ IsoFLOP-профилей:** Для понимания влияния размера учителя и объема его обучения независимо, исследователи использовали **IsoFLOP-профили**. Варьируя размер модели и количество обучающих токенов при фиксированном общем количестве вычислительных операций (FLOPs), они смогли наблюдать, как кросс-энтропия учителя ($L_T$) влияет на кросс-энтропию студента ($L_S$) для студентов фиксированного размера. Эти эксперименты показали, что влияние учителя на ученика описывается степенным законом, который меняется в зависимости от относительных возможностей обучения студента и учителя, что и отражает "capacity gap".
*   **Разрешение вопросов о "capacity gap":** Параметризация закона масштабирования позволила исследователям показать, что "capacity gap" является разрывом в **способности к обучению** (как в пространстве гипотез, так и в способности к оптимизации) между учителем и учеником, а не только в их относительных размерах. Эксперименты с ядреной регрессией и многослойными персептронами (MLP) в Приложении C также статистически демонстрируют этот эффект. Например, в экспериментах с MLP (Приложение C.2) варьировались ширина учителя и объем его обучающих данных, что позволило наблюдать U-образную зависимость производительности студента не только от размера учителя, но и от качества его обучения.
*   **Прогнозирование оптимального учителя:** Закон масштабирования позволяет **прогнозировать** кросс-энтропию студента для различных конфигураций учителя и студента при заданном вычислительном бюджете. Таким образом, можно численно найти такую конфигурацию учителя (размер и объем обучения), которая при заданном размере студента и вычислительном бюджете минимизирует кросс-энтропию студента, что соответствует нахождению **оптимального баланса** и минимизации негативного влияния "capacity gap".

Таким образом, исследователи статистически подошли к определению оптимального разрыва в способности к обучению, разработав **прогностическую модель (закон масштабирования)**, **эмпирически верифицировав ее с помощью контролируемых экспериментов** и показав, как **взаимосвязь между учителем и учеником, отраженная в законе, проявляет U-образное поведение**, указывающее на существование оптимального учителя, который не слишком мал и не слишком велик для данного студента.

---

Дистилляция эффективнее стандартного обучения в двух случаях: когда вычислительные ресурсы для студента ограничены пороговым значением и когда модель-учитель уже существует. Практически, для модели в 1 миллиард параметров дистилляция оптимальна при бюджете менее 20 миллиардов токенов обучения. Если модель-учитель формирует представления, использующие сложные взаимосвязи между сотнями измерений, модель-студент с меньшей размерностью физически не способна воспроизвести эти многомерные связи даже при идеальных сигналах от учителя. Этот закон позволяет компаниям оптимально выбирать размер учителя и экономить миллионы долларов вычислительных ресурсов при создании компактных, но эффективных моделей ИИ.