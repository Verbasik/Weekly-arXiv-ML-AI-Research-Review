...
5. **Нейронная сеть прямого распространения (Feed Forward Network):**
   ...
   - FFN состоит из двух линейных слоев с функцией активации (например, ReLU, GeLU) между ними:
    ...
    - Выход: FFN преобразует вход и выдает тензор **той же размерности** `[batch_size, sequence_length, hidden_size]` ($D_{model}$).  Этот выход затем может быть передан в следующий слой Transformer или использован для решения задачи (например, классификации или генерации текста).
