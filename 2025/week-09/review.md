...
## 2.3 Тренинг дизайна восприятия:

NSA реализует сквозное обучение, сокращая объем предварительных вычислений без ущерба для производительности модели, что позволяет эффективно использовать разреженные модели внимания на протяжении всего жизненного цикла модели.

## 2.4 Система выравнивания оборудования:

Для достижения ускорения уровня FlashAttention во время обучения и предварительного заполнения мы реализованы аппаратно-выровненные разреженные ядра внимания на Triton. Блок разреженного внимания оптимизирован для использования преимуществ тензорных ядер и доступа к памяти, обеспечивая сбалансированную арифметическую интенсивность. В частности, существует несколько оптимизаций, а именно:

- **Режим блочного доступа к памяти:** максимальное использование ядра Tensor и сокращение избыточной передачи KV путем объединения загрузки.

- **Планирование циклов:** разумное планирование циклов в ядре для устранения избыточных передач KV.

- **Загрузка данных, ориентированная на группы:** для каждого внутреннего цикла загружаются все запросы в пределах одной группы и их общий разреженный индекс блока ключей/значений.

- **Совместная выборка KV:** во внутреннем цикле последовательно загружайте последовательные блоки ключей/значений в SRAM, чтобы минимизировать нагрузку на память.

- **Внешний цикл с сетевым планированием:** поскольку длина внутреннего цикла практически одинакова для разных блоков запросов, цикл запроса/вывода помещается в сеточный планировщик Triton для упрощения и оптимизации ядра.
